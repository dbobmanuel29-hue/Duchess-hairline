...
function send(res, status, body, origin) { setHeaders(res, origin); return res.status(status).json(body); }
...
async function lookupCurrentAdmin(idToken) {
  ...
  const response = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=${encodeURIComponent(WEB_API_KEY)}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ idToken }) });
  const data = await response.json();
...
async function getDocument(accessToken, path) {
  const response = await fetch(`https://firestore.googleapis.com/v1/projects/${encodeURIComponent(PROJECT_ID)}/databases/(default)/documents/${path}`, { headers: { Authorization: `Bearer ${accessToken}` } });
...
async function deleteDocument(accessToken, path) {
  const response = await fetch(`https://firestore.googleapis.com/v1/projects/${encodeURIComponent(PROJECT_ID)}/databases/(default)/documents/${path}`, { method: 'DELETE', headers: { Authorization: `Bearer ${accessToken}` } });
  if (!response.ok && response.status !== 404) { const data = await response.json().catch(() => ({})); throw new Error(data.error?.message || 'Could not delete the Firestore profile.'); }
}
async function deleteAuthUser(accessToken, uid) {
  const response = await fetch(`https://identitytoolkit.googleapis.com/v1/projects/${encodeURIComponent(PROJECT_ID)}/accounts:delete`, { method: 'POST', headers: { Authorization: `Bearer ${accessToken}`, 'content-type': 'application/json' }, body: JSON.stringify({ localId: uid }) });
  if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.error?.message || 'Could not delete the Firebase Authentication account.'); }
}
export default async function handler(req, res) {
  const origin = req.headers.origin;
  if (req.method === 'OPTIONS') { setHeaders(res, origin); return res.status(204).end(); }
  ...
  try { await verifyAppCheckToken(typeof appCheckToken === 'string' ? appCheckToken : ''); }
  catch (error) { console.error('App Check verification failed:', error); return send(res, 401, { error: 'Invalid or missing App Check token.' }, origin); }
  let body;
  try { body = typeof req.body === 'string' ? JSON.parse(req.body || '{}') : (req.body || {}); } catch { return send(res, 400, { error: 'Invalid JSON request body.' }, origin); }
  ...
  try {
    ...
    if (adminDoc?.fields?.role?.stringValue !== 'admin' || adminDoc.fields?.active?.booleanValue !== true) return send(res, 403, { error: 'Administrator authorization required.' }, origin);
    if (requestedUid === adminUid) return send(res, 400, { error: 'The current administrator cannot delete their own account.' }, origin);
    const targetAdminDoc = await getDocument(accessToken, `admins/${encodeURIComponent(requestedUid)}`);
    if (targetAdminDoc?.fields?.role?.stringValue === 'admin') return send(res, 403, { error: 'Admin accounts are protected from deletion.' }, origin);
  ...
  } catch (error) { console.error('Admin user deletion failed:', error); return send(res, 500, { error: error instanceof Error ? error.message : 'User deletion failed.' }, origin); }
}
