...
export default function ClientRequest() {
  ...
  if (!user) {
    return <main className="min-h-screen bg-warm-white pt-28 px-4 pb-20"><SeoHead title="Client Request — Duchess Hairline" description="Sign in to send a client request to Duchess Hairline." /><div className="mx-auto max-w-lg rounded-3xl bg-white p-8 shadow-xl text-center"><p className="label-text text-charcoal/50">Duchess Hairline</p><h1 className="mt-3 font-display text-4xl font-light">Send a client request.</h1><p className="mt-4 text-sm leading-7 text-charcoal/60">Sign in with Google or email and password so Duchess Hairline can identify who sent the request and reply to you.</p><button onClick={() => navigate('/login')} className="mt-7 w-full rounded-xl bg-black p-3 text-white">Sign in / Create account</button><Link to="/" className="mt-4 inline-block text-sm underline">Back to store</Link></div></main>;
  }
