...
export default function ProductPage() {
  ...
  const structuredData = useMemo(() => {
    ...
    return { '@context': 'https://schema.org', '@type': 'Product', name: product.name, description: productDescription, image: product.images, sku: product.id, brand: { '@type': 'Brand', name: business.name }, category: product.categoryLabel, url: canonicalUrl, offers: { '@type': 'Offer', url: canonicalUrl, priceCurrency: 'NGN', availability: product.available ? 'https://schema.org/InStock' : 'https://schema.org/OutOfStock', ...(product.price != null ? { price: product.price } : {}) } } as Record<string, unknown>;
  ...
  return <main className="pt-16 md:pt-20 pb-mobile-nav min-h-screen">
    ...
    <div className="max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 pb-16">
        <div>
          <button type="button" onClick={() => setLightboxOpen(true)} className="relative block w-full overflow-hidden bg-cream aspect-[3/4] cursor-zoom-in" aria-label="Open larger image"><img src={product.images[activeImage] || business.name} alt={`${product.name}, image ${activeImage + 1} of ${imageCount}`} className="w-full h-full object-cover" loading="eager" fetchPriority="high" onError={event=>{event.currentTarget.style.display='none';}} />{product.badge && <span className="product-badge absolute top-4 left-4">{product.badge}</span>}</button>
