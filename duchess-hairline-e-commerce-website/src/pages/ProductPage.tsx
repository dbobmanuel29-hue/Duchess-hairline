...
export default function ProductPage() {
  ...
  const specs = [
    { label: 'Length', value: product.length }, { label: 'Hair Type', value: product.hairType }, { label: 'Lace Type', value: product.laceType }, { label: 'Density', value: product.density }, { label: 'Texture', value: product.texture },
  ...
  return <main className="pt-16 md:pt-20 pb-mobile-nav min-h-screen">
    ...
    <div className="max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 pb-16">
        <div>
          <button type="button" onClick={() => setLightboxOpen(true)} className="relative block w-full overflow-hidden bg-cream aspect-[3/4] cursor-zoom-in" aria-label="Open larger image">
            <img src={product.images[activeImage]} alt={`${product.name}, image ${activeImage + 1} of ${imageCount}`} className="w-full h-full object-cover" loading="eager" fetchPriority="high" />
        ...
        <div className="lg:pt-4">
          ...
          <h1 className="font-display text-3xl md:text-4xl lg:text-5xl text-deep-black font-light leading-tight mb-4">{product.name}</h1>
          ...
          <div className="border-t border-beige/30 pt-6 space-y-3"><a href={productOrderLink(product)} target="_blank" rel="noopener noreferrer" className="btn-whatsapp w-full"><WhatsAppIcon size={18} />Order on WhatsApp</a><Link to={routes.collection} className="btn-outline w-full">Continue Browsing</Link><p className="text-xs text-charcoal/40 text-center pt-2">Prices are confirmed on WhatsApp before any order is agreed.</p></div>
        </div>
      </div>
    ...
    {lightboxOpen && <div className="lightbox-overlay" onClick={() => setLightboxOpen(false)} role="dialog" aria-modal="true" aria-label={`${product.name} enlarged image`}>
      <button type="button" onClick={() => setLightboxOpen(false)} className="absolute top-6 right-6 text-white/70 hover:text-white z-10" aria-label="Close image"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M18 6L6 18M6 6l12 12" /></svg></button>
      {imageCount > 1 && <><button type="button" onClick={event => { event.stopPropagation(); setActiveImage(i => (i - 1 + imageCount) % imageCount); }} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/60 hover:text-white p-2 z-10" aria-label="Previous image">‹</button><button type="button" onClick={event => { event.stopPropagation(); setActiveImage(i => (i + 1) % imageCount); }} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/60 hover:text-white p-2 z-10" aria-label="Next image">›</button></>}
      <img src={product.images[activeImage]} alt={product.name} className="max-h-[85vh] max-w-[90vw] object-contain" onClick={event => event.stopPropagation()} />
    </div>}
  </main>;
}
