...
export function generalEnquiryLink(): string {
  return whatsAppLink(
    `Hello ${business.name}, I am browsing your website and would like to know more about your wigs. Please assist me.`,
  );
}
...
export function productOrderLink(product: Product): string {
  ...
  if (typeof window !== 'undefined') {
    lines.push(`Link: ${window.location.origin}${window.location.pathname}#/product/${product.id}`);
  }
...
export function directionsLink(): string {
  return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(business.address.full)}`;
}
