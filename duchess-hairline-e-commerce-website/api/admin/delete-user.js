...
export default async function handler(req, res) {
  ...
  if (!SERVICE_ACCOUNT_JSON) return send(res, 503, { error: 'Server-side Firebase credentials are not configured yet.', code: 'server/missing-service-account' }, origin);
  ...
  try {
    ...
    if (details.code.toLowerCase().includes('permission')) {
      return send(res, 503, { error: 'The server service account cannot verify Firebase App Check tokens. Grant it the Firebase App Check Token Verifier role.', code: details.code }, origin);
    }
  ...
  catch { return send(res, 400, { error: 'Invalid JSON request body.' }, origin); }
  ...
  try {
    ...
    if (adminData?.role !== 'admin' || adminData?.active !== true) return send(res, 403, { error: 'Administrator authorization required.' }, origin);
    ...
    if (targetAdminData?.role === 'admin') return send(res, 403, { error: 'Admin accounts are protected from deletion.' }, origin);

    // Official Firebase Admin SDK deletion flow.
    await auth.deleteUser(requestedUid);

    try { await db.collection('users').doc(requestedUid).delete(); }
    catch (error) { if (error?.code !== 5 && error?.code !== 'not-found') throw error; }

    return send(res, 200, { success: true }, origin);
  } catch (error) {
    const details = errorDetails(error);
    console.error('Admin user deletion failed:', details);
    if (details.code === 'auth/user-not-found') return send(res, 404, { error: 'That Firebase Authentication user no longer exists.', code: details.code }, origin);
    if (details.code === 'auth/id-token-revoked' || details.code === 'auth/user-disabled') return send(res, 401, { error: 'Your administrator session is no longer valid. Please sign in again.', code: details.code }, origin);
    if (details.code === 'auth/insufficient-permission') return send(res, 503, { error: 'The Vercel Firebase service account does not have permission to delete Authentication users. Grant the Firebase Authentication Admin role to that service account, then redeploy.', code: details.code }, origin);
    if (details.code === 'auth/invalid-credential' || details.code === 'auth/project-not-found') return send(res, 503, { error: 'The Vercel Firebase service-account credentials or project configuration are invalid. Check FIREBASE_SERVICE_ACCOUNT_JSON and redeploy.', code: details.code }, origin);
